...
# Snabbpris/TTM-fält från Yahoo
try:
    import yfinance as yf
except Exception:
    yf = None  # handled senare
...
def snapshot_on_start(df: pd.DataFrame, base_ws_title: str):
    """
    Skapa snapshot-blad 1 gång per körning och ta bort äldre än 5 dagar.
    Skyddar mot kvot-/cell-limits och stora datablad.
    """
    if st.session_state.get("_snapshot_done"):
        return
    st.session_state["_snapshot_done"] = True

    # Storleksvakt (minska risken för cell-limit)
    try:
        cells = int(df.shape[0]) * int(df.shape[1])
    except Exception:
        cells = 0
    if cells > 200_000:
        st.sidebar.warning("Hoppar över snapshot (bladet är stort).")
        return
...
TEXT_COLS = ["Ticker","Bolagsnamn","Sektor","Valuta","Auto källa",
...
def normalize_numeric(df: pd.DataFrame) -> pd.DataFrame:
    """Konvertera alla numeriska kolumner robust (komma→punkt, tomma→0)."""
    out = df.copy()
...
# ---------- Snabb Yahoo (pris, valuta, utdelning, CAGR) ----------
...
# ---------- Global SIDOPANEL: Snabb & Full uppdatering ----------
def sidebar_updaters(df: pd.DataFrame, ws_title: str) -> pd.DataFrame:
    st.sidebar.markdown("---")
    st.sidebar.subheader("🔄 Uppdatering")

    delay_s = st.sidebar.slider("Fördröjning mellan hämtningar (sek)", min_value=0.0, max_value=2.0, value=0.5, step=0.1)

    # Snabb (Yahoo – pris/valuta/utdelning/CAGR)
    if st.sidebar.button("⚡ Snabbuppdatering (Yahoo)"):
        if df.empty:
            st.sidebar.warning("Inga rader att uppdatera.")
        else:
            ...
            for idx, r in df.reset_index().iterrows():
                ...
                try:
                    ...
                    if float(q.get("Aktuell kurs",0.0))>0: df.loc[m, "Aktuell kurs"] = float(q["Aktuell kurs"])
                    df.loc[m, "Årlig utdelning"] = float(q.get("Årlig utdelning", 0.0))
                    df.loc[m, "CAGR 5 år (%)"]   = float(q.get("CAGR 5 år (%)", 0.0))
            ...
            if fails:
                st.sidebar.warning("Misslyckade tickers:\n" + ", ".join(fails))

    # Full uppdatering (alla fetchers)
    if st.sidebar.button("🛰️ Full uppdatering (Yahoo + Finviz + Morningstar + SEC)"):
        if df.empty:
            st.sidebar.warning("Inga rader att uppdatera.")
        else:
            status = st.sidebar.empty()
            ...
            for idx, r in df.reset_index().iterrows():
                    ...
                    try:
                        ...
                        for k_src, k_dst in mapping.items():
                            ...
                            if k_dst == "Utestående aktier":
                                df.loc[m, k_dst] = float(v) / 1e6
                            else:
                                df.loc[m, k_dst] = float(v) if isinstance(v, (int,float)) else str(v)
                        df.loc[m, "Senast auto uppdaterad"] = now_stamp()
                        df.loc[m, "Auto källa"] = "Yahoo"
                    except Exception:
                        pass
                ...
                if fz_overview:
                    try:
                        ...
                        if float(fz.get("ps_ttm", 0.0)) > 0: df.loc[m, "P/S"] = float(fz["ps_ttm"])
                        if float(fz.get("pb", 0.0))     > 0: df.loc[m, "P/B"] = float(fz["pb"])
                        df.loc[m, "Senast auto uppdaterad"] = now_stamp()
                        df.loc[m, "Auto källa"] = "Finviz"
                    except Exception:
                        pass
                ...
                if ms_overview:
                    try:
                        ...
                        if float(ms.get("ps_ttm", 0.0)) > 0: df.loc[m, "P/S"] = float(ms["ps_ttm"])
                        if float(ms.get("pb", 0.0))     > 0: df.loc[m, "P/B"] = float(ms["pb"])
                        df.loc[m, "Senast auto uppdaterad"] = now_stamp()
                        df.loc[m, "Auto källa"] = "Morningstar"
                    except Exception:
                        pass
                ...
                if sec_pb_quarters:
                    try:
                        ...
                        if pairs:
                            pairs = pairs[:4]  # Q1..Q4
                            for j, (_, pbv) in enumerate(pairs, start=1):
                                if j > 4: break
                                df.loc[m, f"P/B Q{j}"] = float(pbv or 0.0)
                            # uppdatera snitt
                            row = df.loc[m].iloc[0]
                            _ps_avg, pb_avg = compute_ps_pb_snitt(row)
                            df.loc[m, "P/B-snitt (Q1..Q4)"] = pb_avg
                        df.loc[m, "Senast auto uppdaterad"] = now_stamp()
                        df.loc[m, "Auto källa"] = "SEC"
                    except Exception:
                        pass
...
def view_manual(df: pd.DataFrame, ws_title: str):
    ...
    if not is_new:
        ...
        row = pd.Series({c: (0.0 if c in NUMERIC_COLS else "") for c in FINAL_COLS})
    ...
    with col_up1:
        if not is_new and st.button("🔭 Full uppdatering (alla fetchers) för vald ticker"):
            ...
            if tkr:
                m = (df["Ticker"].astype(str).str.upper() == tkr)
                # Anropa samma logik som i sidopanelen (fast för 1 rad)
                if y_overview:
                    try:
                        y = y_overview(tkr) or {}
                        mapping = {
                    ...
                    try:
                        ...
                        if pairs:
                            ...
                            for j, (_, pbv) in enumerate(pairs, start=1):
                                if j > 4: break
                                df.loc[m, f"P/B Q{j}"] = float(pbv or 0.0)
                            row2 = df.loc[m].iloc[0]
                            _ps_avg, pb_avg = compute_ps_pb_snitt(row2)
                            df.loc[m, "P/B-snitt (Q1..Q4)"] = pb_avg
                        df.loc[m, "Senast auto uppdaterad"] = now_stamp()
                        df.loc[m, "Auto källa"] = "SEC"
                    except Exception:
    ...
    if st.button("💾 Spara"):
        ...
        if exists:
            ...
            base = {c: (0.0 if c in NUMERIC_COLS else "") for c in FINAL_COLS}
            base.update(update)
...
def view_portfolio(df: pd.DataFrame, rates: Dict[str, float]):
    ...
    if df.empty:
        st.info("Ingen data i bladet.")
        return

    port = df.copy()
    for col in ["Antal aktier", "Aktuell kurs", "GAV (SEK)", "Årlig utdelning"]:
    ...
    for col in ["Antal aktier", "Aktuell kurs", "GAV (SEK)", "Årlig utdelning"]:
        port[col] = port[col].map(_to_float_local)
    ...
    def _rate_for(cur: str) -> float:
        key = str(cur or "").upper().strip()
        try:
            return float(rates.get(key, 1.0))
        except Exception:
            return 1.0

    port["Växelkurs"] = port["Valuta"].map(_rate_for)

    port["Värde (SEK)"] = port["Antal aktier"] * port["Aktuell kurs"] * port["Växelkurs"]
    total = float(port["Värde (SEK)"].sum())
    port["Andel (%)"] = np.where(total > 0, (port["Värde (SEK)"] / total) * 100.0, 0.0).round(2)
    ...
    st.markdown(f"**Totalt portföljvärde:** {total:,.2f} SEK".replace(",", " "))
    tot_cost = float(port["Anskaffningsvärde (SEK)"].sum())
    st.markdown(f"**Anskaffningsvärde:** {tot_cost:,.2f} SEK".replace(",", " "))
    tot_pnl = total - tot_cost
    pnl_pct = (tot_pnl / tot_cost * 100.0) if tot_cost > 0 else 0.0
    st.markdown(f"**Vinst/Förlust:** {tot_pnl:,.2f} SEK ({pnl_pct:.2f}%)".replace(",", " "))
    st.markdown(f"**Total årlig utdelning (SEK):** {float(port['Total årlig utdelning (SEK)'].sum()):,.2f}".replace(",", " "))

    desired_cols = [
        "Ticker","Bolagsnamn","Sektor","Antal aktier","GAV (SEK)",
        "Aktuell kurs","Valuta","Växelkurs","Värde (SEK)","Andel (%)",
        "Anskaffningsvärde (SEK)","Vinst (SEK)","Vinst (%)",
        "Årlig utdelning","DA (%)","Total årlig utdelning (SEK)"
    ]
    cols = [c for c in desired_cols if c in port.columns]
    port_sorted = port.sort_values("Andel (%)", ascending=False)
    st.dataframe(port_sorted[cols], use_container_width=True)
...
def main():
    ...
    # Sidopanel: tvångsläs om (om du editerat i Google Sheets)
    ...
    # Sidopanel: Snabb & Full uppdatering
    df = sidebar_updaters(df, ws_title)
