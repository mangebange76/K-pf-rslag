...
SHEET_NAME = "Blad1"
RATES_SHEET_NAME = "Valutakurser"  # separat blad för användarens valutakurser
...
def get_or_create_rates_sheet():
    ...
    try:
        ...
        return ss.add_worksheet(title=RATES_SHEET_NAME, rows=10, cols=2)
...
# ---- Valutakurser (standard + läs/skriv till separat blad) ----
...
def hamta_valutakurser_fran_sheet() -> dict:
    """Läs sparade valutakurser från bladet 'Valutakurser'."""
    out = {}
    try:
        ws = get_or_create_rates_sheet()
        vals = ws.get_all_records()
        for row in vals:
            val = str(row.get("Valuta", "")).upper().strip()
            try:
...
def spara_valutakurser_till_sheet(user_rates: dict):
    """Spara användarens aktuella valutakurser till bladet 'Valutakurser'."""
    try:
        ws = get_or_create_rates_sheet()
        rows = [["Valuta", "Kurs"]]
        for k, v in user_rates.items():
            rows.append([k, float(v)])
        ws.clear()
        ws.update(rows)
    except Exception:
        # mjukt fel — rör inte huvudflödet om vi inte kan skriva
        pass

def hamta_valutakurs(valuta: str, user_rates: dict) -> float:
    if not valuta:
        return 1.0
    # använd användarens inmatade kurser (om finns) eller fallback till standard
...
def beräkna_cagr_från_finansiella(tkr: yf.Ticker) -> float:
    """
    Försök hämta "Total Revenue" per år och räkna CAGR mellan första och sista tillgängliga år.
    Returnerar procent (t.ex. 12.34), eller 0.0 om det inte går.
    """
    try:
        ...
        # sortera kronologiskt på kolumnindex (årsdatum)
...
def hamta_yahoo_fält(ticker: str) -> dict:
    ...
    try:
        ...
        # .info kan ibland kasta/vara tomt – alltid in i try/except
        info = {}
        try:
            info = t.info or {}
        except Exception:
        ...
        # Årlig utdelning per aktie (om tillgänglig)
...
def uppdatera_berakningar(df: pd.DataFrame, user_rates: dict) -> pd.DataFrame:
    ...
    - Projicerar omsättning om 2 & 3 år från 'Omsättning nästa år' med CAGR-klamp:
        >100%  -> 50% används
        <0%    -> 2% används
    ...
    for i, rad in df.iterrows():
        ...
        cagr_raw = float(rad.get("CAGR 5 år (%)", 0.0))
        just_cagr = 50.0 if cagr_raw > 100.0 else (2.0 if cagr_raw < 0.0 else cagr_raw)
        g = just_cagr / 100.0
...
# ---- Massuppdatera från Yahoo (1s delay, kopierbar felrapport, sparar även valutakurser) ----
def massuppdatera(df: pd.DataFrame, key_prefix: str, user_rates: dict) -> pd.DataFrame:
    ...
    if st.sidebar.button("🔄 Uppdatera alla från Yahoo", key=f"{key_prefix}_massupd_btn"):
        ...
        for i, row in df.iterrows():
            ...
            # Skriv endast om vi verkligen fått något värde
            ...
            # Årlig utdelning kan vara 0 – sätt 0.0 om None
        ...
        # Spara både data och nuvarande valutakurser
        spara_data(df)
        spara_valutakurser_till_sheet(user_rates)
...
# ---- Fält som räknas som "manuellt uppdaterade" för datumkolumnen ----
...
def lagg_till_eller_uppdatera(df: pd.DataFrame, user_rates: dict) -> pd.DataFrame:
    ...
    etikett_till_ticker = {lab: t for lab, t in zip(etiketter, vis_df["Ticker"])}

    # Håll reda på vilket index vi visar/redigerar
    ...
    current_select_index = 0 if len(etiketter) == 0 else (st.session_state.edit_index + 1)

    valt_label = st.selectbox("Välj bolag (eller 'nytt bolag')", val_lista, index=current_select_index, key="form_select")
    ...
    with col_prev:
        if st.button("⬅️ Föregående"):
            if len(etiketter) > 0:
                ...
                st.session_state.form_select = val_lista[st.session_state.edit_index + 1]
    with col_pos:
        total = max(1, len(etiketter))
        if len(etiketter) > 0 and valt_label != "(nytt bolag)":
            st.write(f"Post {st.session_state.edit_index + 1}/{total}")
        else:
            st.write("Nytt bolag")
    with col_next:
    ...
    # Befintlig rad
    if valt_label != "(nytt bolag)" and len(etiketter) > 0:
        vald_ticker = etikett_till_ticker.get(valt_label, "")
        bef = df[df["Ticker"] == vald_ticker].iloc[0] if not df[df["Ticker"] == vald_ticker].empty else pd.Series({}, dtype=object)
    else:
    ...
    if spar and ticker:
        ...
        if data.get("Aktuell kurs", 0):  df.loc[df["Ticker"] == ticker, "Aktuell kurs"] = data["Aktuell kurs"]
...
def analysvy(df: pd.DataFrame, user_rates: dict) -> None:
    ...
    # Välj via index eller rullista
    st.session_state.analys_idx = st.number_input(
        "Visa bolag #",
        min_value=0, max_value=len(etiketter) - 1,
    ...
    # Fånga bara kolumner som faktiskt finns
...
def visa_portfolj(df: pd.DataFrame, user_rates: dict) -> None:
    ...
    # SEK-värden för portfölj (endast här sker valutakonvertering för visning)
...
def visa_investeringsforslag(df: pd.DataFrame, user_rates: dict) -> None:
    ...
    # Potential och “avvikelse mot mål”
    ...
    # Portföljvärde för andelsberäkning (SEK)
    port = df[df["Antal aktier"] > 0].copy()
    if not port.empty:
        port["Växelkurs"] = port["Valuta"].apply(lambda v: hamta_valutakurs(v, user_rates))
        port["Värde (SEK)"] = port["Antal aktier"] * port["Aktuell kurs"] * port["Växelkurs"]
        port_värde = float(port["Värde (SEK)"].sum())
        nuv_innehav = float(port[port["Ticker"] == rad["Ticker"]]["Värde (SEK)"].sum())
    else:
        port_värde = 0.0
        nuv_innehav = 0.0

    # Hur många kan köpas för kapital_sek (konvertera endast för köpe-beräkning)
    ...
    # Presentationskort (visa alla riktkurser — den valda fetas)
...
def main():
    ...
    # Global sidopanel: massuppdatera från Yahoo (1s delay + felrapport)
    df = massuppdatera(df, key_prefix="global", user_rates=user_rates)

    # Global “spara nu”-knapp
    if st.sidebar.button("💾 Spara allt nu"):
        spara_data(df)
        st.sidebar.success("Allt sparat.")

    meny = st.sidebar.radio("📌 Välj vy", ["Analys", "Lägg till / uppdatera bolag", "Investeringsförslag", "Portfölj"])

    if meny == "Analys":
        # Räkna om inför visning (ifall kurser etc uppdaterats)
        df = uppdatera_berakningar(df, user_rates)
