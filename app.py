# ============================================================
# app.py  —  monolit med svensk decimalhantering + GS + Yahoo
# ============================================================
...
def spara_data(df: pd.DataFrame):
    """Skriv tillbaka som värden (inte tvångssträngar) så behåller GS rätt decimallokal."""
    sheet = skapa_koppling()
...
def las_sparade_valutakurser_cached(nonce: int):
    """Läs sparade kurser från bladet 'Valutakurser'."""
...
def spara_valutakurser(rates: dict):
    """Spara kurser i bladet 'Valutakurser' som tal (ej tvingade strängar)."""
    ws = skapa_rates_sheet_if_missing()
    body = [["Valuta","Kurs"]]
...
def massuppdatera(df: pd.DataFrame, key_prefix: str, user_rates: dict) -> pd.DataFrame:
    ...
    if st.sidebar.button("🔄 Uppdatera alla från Yahoo", key=f"{key_prefix}_massupd_btn"):
        ...
        for i, row in df.iterrows():
            ...
            time.sleep(1.0)  # försiktigt mot Yahoo-rate limits
...
# =========================
# LÄGG TILL / UPPDATERA BOLAG (med dubblettkontroll)
# =========================
def lagg_till_eller_uppdatera(df: pd.DataFrame, user_rates: dict) -> pd.DataFrame:
    ...
    if spar and ticker:
        ...
        # Sätt “Senast manuellt uppdaterad” om relevanta fält ändrats / ifyllda
        datum_sätt = False
        if not bef.empty:
        ...
        if not bef.empty:
            # uppdatera raden (obs: använd gamla nyckeln cur_tkr om den ännu finns i df)
            key_tkr = new_tkr if (df["Ticker"].astype(str).str.upper()==new_tkr).any() else cur_tkr
            for k,v in ny.items():
                df.loc[df["Ticker"].astype(str).str.upper()==key_tkr, k] = v
            # om användaren bytte från cur_tkr → new_tkr, se till att fältet Ticker byts
            if new_tkr != cur_tkr:
    ...
    # Lista: äldst manuellt uppdaterade (topp 10)
...
# =========================
# INVESTERINGSFÖRSLAG (med P/S-filter + visning av P/S & P/S-snitt)
# =========================
def visa_investeringsforslag(df: pd.DataFrame, user_rates: dict) -> None:
    ...
    # Visa även P/S (nuvarande) och P/S-snitt
    st.subheader(f"{rad['Bolagsnamn']} ({rad['Ticker']})")
    st.markdown(
        f"""
- **Aktuell kurs:** {round(rad['Aktuell kurs'],2)} {rad['Valuta']}
- **Nuvarande P/S (TTM):** {round(float(rad.get('P/S', 0.0)), 2)}
- **P/S-snitt (Q1–Q4):** {round(float(rad.get('P/S-snitt', 0.0)), 2)}
...
def main():
    ...
    # 1) Tyst auto-uppdatering av valutakurser om sparade värden är stale
    auto_update_valutakurser_if_stale()

    # 2) Sidopanel: valutakurser → läs sparade och visa inputs
    ...
    # Manuell live-hämtning från Yahoo och direkt sparning
    ...
    # 3) Läs data + säkerställ schema/typer
    df = hamta_data()
    if df.empty:
    ...
    # Om valutakurserna nyss sparats/live-hämtats → räkna om riktkurser & spara
    if st.session_state.get("recalc_after_rates", False):
        df = uppdatera_berakningar(df, user_rates)
        spara_data(df)
        st.session_state["recalc_after_rates"] = False

    # 4) Global massuppdatering från Yahoo (namn, kurs, valuta, utd, CAGR)
