...
# ---------- robust siffertolkning (komma/punkt) ----------
def _to_float(x) -> float:
    ...
    if not s or s.lower() in {"nan", "none"}:
        return 0.0
    s = s.replace(" ", "")
    # Både komma & punkt – sista symbolen antas vara decimaltecken
    if "," in s and "." in s:
        if s.rfind(",") > s.rfind("."):
            s = s.replace(".", "").replace(",", ".")
        else:
...
def ensure_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in float_cols:
        out[c] = pd.to_numeric(out[c].apply(_to_float), errors="coerce").fillna(0.0)
...
# ---------- Legacy → nya kolumner (viktig fix) ----------
def migrate_legacy_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Mappar gamla kolumnnamn till nya och sanerar 'Utestående aktier'."""
    out = df.copy()
    rename_map = {
        "Kurs": "Aktuell kurs",
        "Price": "Aktuell kurs",
        "Utestående aktier (M)": "Utestående aktier",
        "Outstanding shares (M)": "Utestående aktier",
        "P/S-snitt": "P/S-snitt (Q1..Q4)",
        "P/S snitt": "P/S-snitt (Q1..Q4)",
        "PS-snitt": "P/S-snitt (Q1..Q4)",
        "Årlig utd": "Årlig utdelning",
        "Dividend/år": "Årlig utdelning",
        "Currency": "Valuta",
        "Sector": "Sektor",
        "Shares": "Antal aktier",
        "GAV": "GAV (SEK)",
    ...
    for old, new in rename_map.items():
        if old in out.columns:
            if new not in out.columns:
                ...
                # fyll där nya är tom/0
                if new in ["Ticker","Bolagsnamn","Sektor","Valuta"]:
                    mask = out[new].astype(str).str.strip().eq("")
                else:
                    mask = out[new].map(_to_float).eq(0.0)
                out.loc[mask, new] = out[old]
                out = out.drop(columns=[old])

    # Utestående aktier – om verkar vara absoluta tal → gör om till miljoner
    if "Utestående aktier" in out.columns:
        vals = out["Utestående aktier"].map(_to_float)
        pos = vals[vals > 0]
        if len(pos) and np.median(pos) > 1e8:  # sannolikt absoluta
            out["Utestående aktier"] = (vals / 1e6).round(6)
        else:
            out["Utestående aktier"] = vals
    return out
...
def load_df(ws_title: str) -> pd.DataFrame:
    ...
    # FÖRST: migrera legacy-kolumner
    raw = migrate_legacy_columns(raw)
    # SEN: säkra kolumner & numeriska typer
...
def _update_one_all_fetchers(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
        ...
        try:
            ...
            for k_src, k_dst in mapping.items():
                ...
                if k_dst == "Utestående aktier":
                    ...
                    df.loc[mask, k_dst] = _to_float(v) if isinstance(v, (int,float)) else str(v)
    ...
    if fz_overview:
        try:
            ...
            if _to_float(fz.get("ps_ttm", 0.0)) > 0: df.loc[mask, "P/S"] = _to_float(fz["ps_ttm"])
            if _to_float(fz.get("pb", 0.0))     > 0: df.loc[mask, "P/B"] = _to_float(fz["pb"])
            df.loc[mask, "Senast auto uppdaterad"] = now_stamp()
    ...
    if ms_overview:
        try:
            ...
            if _to_float(ms.get("ps_ttm", 0.0)) > 0: df.loc[mask, "P/S"] = _to_float(ms["ps_ttm"])
            if _to_float(ms.get("pb", 0.0))     > 0: df.loc[mask, "P/B"] = _to_float(ms["pb"])
            df.loc[mask, "Senast auto uppdaterad"] = now_stamp()
        ...
        try:
            ...
            if pairs:
                ...
                for idx, (_, pbv) in enumerate(pairs, start=1):
                    if idx > 4: break
                    df.loc[mask, f"P/B Q{idx}"] = _to_float(pbv or 0.0)
...
def view_portfolio(df: pd.DataFrame, rates: Dict[str, float]):
    ...
    port["Vx"] = port["Valuta"].astype(str).str.upper().map(lambda v: float(rates.get(v, 1.0)))
    ...
    res = tot_val - tot_cost
    res_pct = (res / tot_cost * 100.0) if tot_cost > 0 else 0.0
    ...
    tot_yr = float(port["Årlig utd (SEK)"].sum())

    st.markdown(f"- **Totalt portföljvärde:** {round(tot_val,2)} SEK")
    st.markdown(f"- **Anskaffningsvärde:** {round(tot_cost,2)} SEK")
    st.markdown(f"- **Resultat:** {round(res,2)} SEK ({round(res_pct,2)} %)")
    st.markdown(f"- **Total årlig utdelning:** {round(tot_yr,2)} SEK  (≈ {round(tot_yr/12.0,2)} SEK/mån)")

    cols = ["Ticker","Bolagsnamn","Sektor","Antal aktier","GAV (SEK)","Aktuell kurs","Valuta","Värde (SEK)","Anskaffning (SEK)","Resultat (SEK)","Årlig utdelning","Årlig utd (SEK)","DA (%)","Andel (%)"]
