...
# (valfria) fetchers – appen kör dem bara om de finns i miljön
try:
...
FINAL_COLS: List[str] = [
    ...
    # Multiplar P/S & P/B (kvartal + snitt)
...
# Kolumn-alias → mål (löser t.ex. ”Antal Aktier”, ”Omsättning i år”)
CANONICAL_ALIASES = {
    "Antal aktier": ["Antal Aktier", "antal aktier", "Antal aktier du äger"],
    "Omsättning idag": ["Omsättning i år", "Omsättning iår", "Omsättning idag (M)"],
    "Omsättning nästa år": ["Omsättning nästa år (M)", "Omsättning nästa År"],
    "GAV (SEK)": ["GAV", "GAV SEK", "GAV( SEK )"],
}
...
NUMERIC_TARGETS = {
    ...
    "Utestående aktier", "P/S","P/S Q1","P/S Q2","P/S Q3","P/S Q4","P/S-snitt (Q1..Q4)",
    "P/B","P/B Q1","P/B Q2","P/B Q3","P/B Q4","P/B-snitt (Q1..Q4)",
    "Riktkurs idag","Riktkurs om 1 år","Riktkurs om 2 år","Riktkurs om 3 år",
    "CAGR 5 år (%)","Payout (%)",
    "DA (%)", "Uppsida idag (%)", "Uppsida 1 år (%)","Uppsida 2 år (%)","Uppsida 3 år (%)",
...
def normalize_and_coalesce_columns(df: pd.DataFrame) -> pd.DataFrame:
    # 0) normalisera whitespace
    df = df.rename(columns=lambda c: str(c).strip())

    # 1) Bygg rename-map (case-insensitiv match mot alias)
    rename_map = {}
    ...
    if rename_map:
        ...
        st.caption("ℹ️ Normaliserade rubriker: " + ", ".join([f"{k}→{v}" for k, v in rename_map.items()]))

    # 2) Slå ihop aliasvärden in i target
    for target, aliases in CANONICAL_ALIASES.items():
        if target not in df.columns:
            df[target] = 0.0 if target in NUMERIC_TARGETS else ""
        if target in NUMERIC_TARGETS:
            df[target] = df[target].apply(_to_float)
        for al in aliases:
            if al in df.columns and al != target:
                if target in NUMERIC_TARGETS:
                    df[al] = df[al].apply(_to_float)
                    df[target] = df[target].where(df[target] != 0.0, df[al])
                else:
                    df[target] = df[target].where(df[target].astype(str).str.len() > 0, df[al])

    # 3) Säkerställ slutliga kolumner & typer
    df = ensure_columns(df)
    return df

# ---------------- Snapshot on start (med skydd mot cellgräns) ----------------
def snapshot_on_start(df: pd.DataFrame, base_ws_title: str, max_cells: int = 300_000):
    """
    Skapar snapshotblad vid första körning och rensar äldre än 5 dagar.
    Vi skippar snapshot om df är alltför stort (cells > max_cells).
    """
    ...
    try:
        ...
        cutoff = _now_sthlm() - timedelta(days=5)
        for t in titles:
        ...
        st.sidebar.warning(f"Kunde inte rensa snapshots: {e}")
...
# ---------- IO (med cache-nonce + normalisering) ----------
...
def load_df(ws_title: str) -> pd.DataFrame:
    ...
    # Normalisera rubriker och slå ihop alias → rätt fält hamnar rätt
    df = normalize_and_coalesce_columns(df)
    # Säkerställ att alla slutliga kolumner finns och har rätt typer
    df = ensure_columns(df)
    return df
...
def compute_ps_pb_snitt(row: pd.Series) -> Tuple[float, float]:
    ...
    pb_clean = [float(x) for x in pb_vals if _to_float(x) > 0]
...
def score_rows(df: pd.DataFrame, horizon: str, strategy: str) -> pd.DataFrame:
    out = df.copy()
    price = out["Aktuell kurs"].map(_to_float)
    div   = out["Årlig utdelning"].map(_to_float)
    out["DA (%)"] = np.where(price > 0, (div / price) * 100.0, 0.0)
    rk = out[horizon].map(_to_float)
    out["Uppsida (%)"] = np.where(price > 0, (rk - price) / price * 100.0, 0.0)
...
try:
    ...
    yf = None  # appen fungerar ändå men snabbhämtning uteblir
...
def yahoo_fetch_one_quick(ticker: str) -> Dict[str, float | str]:
    ...
    try:
        ...
        # Enkel CAGR 5y från total revenue (årliga)
        try:
...
# ---------- Enskild full uppdatering (alla fetchers för en ticker) ----------
def _update_one_all_fetchers(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    ...
    # 4) SEC → P/B kvartal (om tillgänglig)
    if sec_pb_quarters:
        try:
...
def view_manual(df: pd.DataFrame, ws_title: str):
    ...
    if not is_new:
        ...
        # skapa tom serie med alla kolumner
    ...
    # Obligatoriska fält (de du ansvarar för)
    ...
    with c1:
        ...
        antal  = st.number_input(
            ...
            value=_to_float(row.get("Antal aktier",0.0) or 0.0),
            step=1.0, min_value=0.0
        )
        gav    = st.text_input(
            f"GAV (SEK) per aktie {mtag}",
            value=str(row.get("GAV (SEK)","") or ""),
            help="Ange per aktie. Komma eller punkt accepteras."
        )
    with c2:
        oms_idag = st.text_input(
            f"Omsättning idag (M) {mtag}",
            value=str(row.get("Omsättning idag","") or ""),
            help="Miljoner i bolagets valuta. Komma eller punkt accepteras."
        )
        oms_nxt  = st.text_input(
            f"Omsättning nästa år (M) {mtag}",
            value=str(row.get("Omsättning nästa år","") or ""),
            help="Miljoner i bolagets valuta. Komma eller punkt accepteras."
        )

    # Övriga fält: FETCHERS (kan korrigeras manuellt)
    atag = _a_tag(row)
    with st.expander("🌐 Fält som hämtas (auto)"):
        ...
        with cL:
            ...
            aktuell_kurs = st.text_input(f"Aktuell kurs {atag}", value=str(row.get("Aktuell kurs","") or "0"))
            utd_arlig    = st.text_input(f"Årlig utdelning {atag}", value=str(row.get("Årlig utdelning","") or "0"))
            payout_pct   = st.text_input(f"Payout (%) {atag}", value=str(row.get("Payout (%)","") or "0"))
        with cR:
            utest_m = st.text_input(f"Utestående aktier (miljoner) {atag}", value=str(row.get("Utestående aktier","") or "0"))
            ps  = st.text_input(f"P/S {atag}",   value=str(row.get("P/S","") or "0"))
            ps1 = st.text_input(f"P/S Q1 {atag}", value=str(row.get("P/S Q1","") or "0"))
            ps2 = st.text_input(f"P/S Q2 {atag}", value=str(row.get("P/S Q2","") or "0"))
            ps3 = st.text_input(f"P/S Q3 {atag}", value=str(row.get("P/S Q3","") or "0"))
            ps4 = st.text_input(f"P/S Q4 {atag}", value=str(row.get("P/S Q4","") or "0"))
            pb  = st.text_input(f"P/B {atag}",   value=str(row.get("P/B","") or "0"))
            pb1 = st.text_input(f"P/B Q1 {atag}", value=str(row.get("P/B Q1","") or "0"))
            pb2 = st.text_input(f"P/B Q2 {atag}", value=str(row.get("P/B Q2","") or "0"))
            pb3 = st.text_input(f"P/B Q3 {atag}", value=str(row.get("P/B Q3","") or "0"))
            pb4 = st.text_input(f"P/B Q4 {atag}", value=str(row.get("P/B Q4","") or "0"))
    ...
    if st.button("💾 Spara"):
        ...
        if _to_float(antal) < 0:
            errors.append("Antal aktier kan inte vara negativt.")
        if _to_float(oms_idag) < 0 or _to_float(oms_nxt) < 0:
        ...
        # Samla uppdateringar (med robust float-parsning)
        update = {
            # manuella
            "Ticker": ticker.upper(),
        ...
        if exists:
            ...
            base = {c: (0.0 if c not in [
                ...
                "Senast beräknad","Div_Månader","Div_Vikter"
        ...
        try:
            ...
            # säkerställ att alla slutkolumner finns och rätt typer innan skriv
...
def view_portfolio(df: pd.DataFrame, rates: Dict[str, float]):
    ...
    port["Valuta"]         = port["Valuta"].astype(str).str.upper().replace("", "SEK")

    port = port[port["Antal aktier"] > 0].copy()
    if port.empty:
        st.info("Du äger inga aktier (kolumnen **Antal aktier** är 0 för alla).")
        return
    ...
    # utdelning
    port["Total årlig utd (SEK)"] = port["Antal aktier"] * port["Årlig utdelning"] * port["Växelkurs"]
    tot_div_sek = float(port["Total årlig utd (SEK)"].sum())

    # DA på dagens kurs (egen beräkning)
    port["DA (%)"] = np.where(
        port["Aktuell kurs"] > 0,
    ...
    port["Andel (%)"] = np.where(
        total_value > 0,
        (port["Värde (SEK)"] / total_value) * 100.0,
        0.0
    ).round(2)

    # summering
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Portföljvärde (SEK)", f"{total_value:,.0f}".replace(",", " "))
    c2.metric("Anskaffningsvärde (SEK)", f"{total_cost:,.0f}".replace(",", " "))
    c3.metric("Resultat (SEK)", f"{total_pnl:,.0f}".replace(",", " "), delta=f"{total_pnl_pct:.2f}%")
    c4.metric("Årlig utdelning (SEK)", f"{tot_div_sek:,.0f}".replace(",", " "))

    st.dataframe(
        port[[
            "Ticker","Bolagsnamn","Sektor","Antal aktier",
            "Aktuell kurs","Valuta","Växelkurs",
            "GAV (SEK)","Anskaffning (SEK)","Värde (SEK)",
            "Årlig utdelning","Total årlig utd (SEK)","DA (%)","Andel (%)"
...
def view_ideas(df: pd.DataFrame):
    ...
    with c2:
        ...
        st.write(
            ...
            f"{round(_to_float(r['Score (Growth)']),1)} / {round(_to_float(r['Score (Dividend)']),1)} / "
            f"{round(_to_float(r['Score (Financials)']),1)} / **{round(_to_float(r['Score (Total)']),1)}** "
...
def _sidebar_bulk_update(ws_title: str, df: pd.DataFrame):
    st.sidebar.markdown("### 🔄 Massuppdatering")

    # 1) Snabb (Yahoo: pris, valuta, namn, utdelning, CAGR)
    if st.sidebar.button("⚡ Snabbuppdatera kurser (Yahoo)"):
        try:
            base = df.copy()
            tickers = base["Ticker"].astype(str).str.upper().tolist()
            n = len(tickers)
            if n == 0:
                st.sidebar.info("Inga tickers i bladet.")
                return

            prog = st.sidebar.progress(0.0, text="Startar snabbuppdatering…")
            ...
            for i, tkr in enumerate(tickers, start=1):
                ...
                try:
                    ...
                    mask = (base["Ticker"].astype(str).str.upper() == tkr)
                    if not mask.any():
                        prog.progress(i / n)
                        continue

                    if quick.get("Bolagsnamn"): base.loc[mask, "Bolagsnamn"] = str(quick["Bolagsnamn"])
                    if quick.get("Valuta"):     base.loc[mask, "Valuta"]     = str(quick["Valuta"]).upper()
                    if float(quick.get("Aktuell kurs", 0.0)) > 0:
                        base.loc[mask, "Aktuell kurs"] = float(quick["Aktuell kurs"])
                    base.loc[mask, "Årlig utdelning"] = float(quick.get("Årlig utdelning", 0.0))
                    base.loc[mask, "CAGR 5 år (%)"]   = float(quick.get("CAGR 5 år (%)", 0.0))

                    base.loc[mask, "Senast auto uppdaterad"] = now_stamp()
                    base.loc[mask, "Auto källa"] = "Yahoo (snabb)"
                except Exception as e:
                    fails.append(f"{tkr}: {e}")

                status.write(f"{i}/{n} uppdaterade…")
                prog.progress(i / n)
                time.sleep(0.5)  # artig paus

            base = ensure_columns(base)
            base = enrich_for_save(base, horizon_for_score="Riktkurs idag", strategy="Auto")
            save_df(ws_title, base, bust_cache=True)
            st.sidebar.success("Snabbuppdatering klar och sparad.")
            if fails:
                st.sidebar.warning("Vissa tickers misslyckades:")
                st.sidebar.text_area("Fel", "\n".join(fails), height=120)
            st.rerun()
        except Exception as e:
            st.sidebar.error(f"Kunde inte köra snabbuppdatering: {e}")

    # 2) Full (Yahoo+Finviz+Morningstar+SEC där tillgängligt)
    if st.sidebar.button("🌐 Full uppdatering (alla fetchers)"):
        try:
            ...
            prog = st.sidebar.progress(0.0, text="Startar full uppdatering…")
            status = st.sidebar.empty()
            fails = []
            ...
            for i, tkr in enumerate(tickers, start=1):
                if not tkr:
                    ...
                    continue
                try:
                    base = _update_one_all_fetchers(base, tkr)
                except Exception as e:
                    fails.append(f"{tkr}: {e}")
                status.write(f"{i}/{n} uppdaterade…")
                prog.progress(i / n)
                time.sleep(0.6)  # artig paus (lite längre)
...
def main():
    ...
    # Snapshot (försöker, men tyst felhantering och ingen snapshot om bladet blir för stort i sheets.py)
    try:
        snapshot_on_start(df, ws_title)
    except Exception as e:
        st.sidebar.warning(f"Snapshot hoppades över: {e}")

    # Beräkna här en gång så vyer får färska derivat
    df = update_calculations(df)

    # Massuppdateringsknappar i sidopanelen (återinförda)
    _sidebar_bulk_update(ws_title, df)
