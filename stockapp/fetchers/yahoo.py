...
def get_all(ticker: str) -> dict:
    out = {"name": "", "price": 0.0, "currency": "USD", "dividend": 0.0, "cagr5": 0.0}
    try:
        t = yf.Ticker(ticker)
        info = {}
        try:
        ...
        if valuta: out["currency"] = str(valuta).upper()
        namn = info.get("shortName") or info.get("longName") or ""
        if namn: out["name"] = str(namn)
        div_rate = info.get("dividendRate")
        if div_rate is not None: out["dividend"] = float(div_rate)
